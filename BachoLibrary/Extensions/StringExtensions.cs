...
namespace BachoLibrary.Extensions
{
    ...
    {
        public static bool HasValue(this string text) => !string.IsNullOrEmpty(text);// || Regex.IsMatch(text, @"^(\s|\t|\n)*$");
        ...
        public static string RemoveAllSpaces(this string text) => Regex.Replace(text, @"\s+", string.Empty);
    }
}